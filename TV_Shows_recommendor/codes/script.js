...
// Modify the code where movie cards are created to add a click event listener
movieCard.addEventListener("click", function () {
    openMovieModal(movie);
});
...
function openMovieDetailsPage(movie) {
    ...
    const detailsPageUrl = `movie_details.html?name=${encodeURIComponent(movie.name)}&posterPath=${encodeURIComponent(movie.poster_path)}&genre=${encodeURIComponent(movie.genres)}`;
